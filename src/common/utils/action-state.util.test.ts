...
describe("action response errors", () => {
  ...
  it("returns no error state for successful responses", async () => {
    await expect(
      getResponseErrorActionState(new Response(null, { status: 204 }), fields, "Fallback"),
  ...
  it("uses the fallback for network and malformed response errors", async () => {
    await expect(
      getResponseErrorActionState(Promise.reject(new Error("offline")), fields, "Fallback"),
    ).resolves.toEqual({
      error: "Fallback",
    });

    await expect(
      getResponseErrorActionState(new Response("not-json", { status: 500 }), fields, "Fallback"),
    ).resolves.toEqual({
      error: "Fallback",
    });
  });
