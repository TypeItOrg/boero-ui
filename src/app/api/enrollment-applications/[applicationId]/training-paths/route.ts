...
export async function GET(request: Request, { params }: { params: Promise<{ applicationId: string }> }) {
  ...
  try {
    return createPassthroughResponse(
      await institutionalApiFetch(`/api/v1/enrollment-applications/${applicationId}/training-paths`, { signal: request.signal }),
    );
  } catch {
    return Response.json({ message: ENROLLMENT_MESSAGES.TRAINING_PATHS_UNAVAILABLE }, { status: 503 });
  }
}
