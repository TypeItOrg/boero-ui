...
describe("GET /api/institutional/search", () => {
  ...
  it("rejects an invalid institution identifier", async () => {
    const response = await GET(
      new Request("http://localhost/api/institutional/search?institutionId=invalid&search=rol"),
    );
  ...
  it("forwards only validated values to the scoped backend endpoint", async () => {
    ...
    const request = new Request(
      `http://localhost/api/institutional/search?institutionId=${institutionId}&search=rol&limit=4`,
    );
    ...
    expect(institutionalApiFetchMock).toHaveBeenCalledWith(
      `/api/v1/institutions/${institutionId}/search?search=rol&limit=4`,
  ...
  it("preserves backend authorization failures", async () => {
    institutionalApiFetchMock.mockResolvedValue(
      Response.json(
        ...
        { status: 401, headers: { "x-request-id": "request-institutional-search" } },
      ),
    );
    const response = await GET(
      new Request(`http://localhost/api/institutional/search?institutionId=${institutionId}&search=rol`),
    );
