...
type MyScheduleEventProps = {
  ...
  schedule: CourseEnrollment["schedules"][number];
  dayLabel: string;
  duration: number;
  style: CSSProperties;
};

export function MyScheduleEvent({ enrollment, schedule, dayLabel, duration, style }: MyScheduleEventProps): React.ReactElement {
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <button
          type="button"
          aria-label={`${enrollment.academicSpaceName}, ${dayLabel}, ${time}${enrollment.instrumentName ? `, ${enrollment.instrumentName}` : ""}`}
          className={cn(
        ...
        >
          ...
          {!compact && duration >= 40 && enrollment.instrumentName ? (
            <span className="mt-0.5 block shrink-0 truncate text-xs leading-4">{enrollment.instrumentName}</span>
          ) : null}
        </button>
      </PopoverTrigger>
      <PopoverContent collisionPadding={16} className="w-80 max-w-[calc(100vw-2rem)] p-4" aria-label={enrollment.academicSpaceName}>
        <h3 className="text-base leading-snug font-semibold wrap-break-word">{enrollment.academicSpaceName}</h3>
        <p className="text-primary text-sm font-medium">
          {dayLabel} · {time}
        </p>
        {enrollment.instrumentName ? <p className="text-muted-foreground text-sm">{enrollment.instrumentName}</p> : null}
        <p className="text-muted-foreground text-sm wrap-break-word">
          {[enrollment.trainingPathName, enrollment.studyPlanName, enrollment.academicLevelName].filter(Boolean).join(" · ")}
        </p>
      </PopoverContent>
    </Popover>
  );
