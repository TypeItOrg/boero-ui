...
describe("academic scope", () => {
  ...
  it("builds institutional API and page paths", () => {
    expect(getAcademicApiBase(AcademicScope.INSTITUTIONAL, institutionId)).toBe(
      `/api/v1/institutions/${institutionId}`,
    );
    expect(getAcademicRouteBase(AcademicScope.INSTITUTIONAL, institutionId)).toBe("");
    expect(getAcademicResourceRoute(AcademicScope.INSTITUTIONAL, institutionId, AcademicResource.ACADEMIC_YEAR)).toBe(
      "/academic-years",
    );
    expect(getAcademicResourceRoute(AcademicScope.INSTITUTIONAL, institutionId, AcademicResource.ACADEMIC_LEVEL)).toBe(
      "/study-plans",
    );
  });
  ...
  it("builds platform administration API and page paths", () => {
    ...
    expect(getAcademicRouteBase(AcademicScope.ADMIN, institutionId)).toBe(
