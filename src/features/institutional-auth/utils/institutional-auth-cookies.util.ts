...
export async function setInstitutionalAuthCookies(
  tokens: InstitutionalLoginResult["tokens"],
  rememberMe: boolean,
): Promise<void> {
  ...
  cookieStore.set(INSTITUTIONAL_REFRESH_TOKEN_COOKIE, tokens.refreshToken, {
    ...getInstitutionalAuthCookieOptions(
      rememberMe ? INSTITUTIONAL_REMEMBER_ME_MAX_AGE : INSTITUTIONAL_REFRESH_TOKEN_MAX_AGE,
    ),
  });
}
...
export async function setInstitutionalRegistrationSuccessCookie(): Promise<void> {
  const cookieStore = await cookies();
  cookieStore.set(
    INSTITUTIONAL_REGISTRATION_SUCCESS_COOKIE,
    "true",
    getInstitutionalAuthCookieOptions(INSTITUTIONAL_REGISTRATION_SUCCESS_MAX_AGE),
  );
}
...
export async function setInstitutionalPasswordChangedCookie(): Promise<void> {
  ...
  cookieStore.set(
    ...
    "true",
    getInstitutionalAuthCookieOptions(INSTITUTIONAL_PASSWORD_CHANGED_MAX_AGE),
  );
}
